...
FUNC_CUSTOM_CLEAN()
{
	...
	rm -f build/gracelte/dtb
	rm -f build/gracelte/zImage
	rm -f build/gracelte/.version
...
FUNC_BUILD_DTIMAGE_TARGET()
{
	...
	if [ $MODEL == gracelte ] && [ $VARIANT == kor ]; then
...
FUNC_BUILD_ZIP()
{
	...
	echo ""
	zip -r9 $EXTDIR/${KERNEL_NAME}-v${KERNEL_VERSION}${BUILD_TYPE}-${MODEL}${VARIANT}.zip * .version -x modules/\* -x ramdisk/\*
